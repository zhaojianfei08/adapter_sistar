                ...
                try:
                    ...
                    for column_name, column_type in v['columns']:
                        if column_type == 'datetime':
                            ...
                            column_sql_str += f'CONVERT(VARCHAR, SWITCHOFFSET({column_name}, "+08:00"), 120) AS {column_name}'
                        else:
                            column_sql_str += f'{column_name},'
                    column_sql_str = column_sql_str.strip()[:-1]
                    result = mssql_conn.execute(text(
                        f"select {column_sql_str} from {k} where {v['select_column']} > '{v['last_time']}' ORDER BY {v['select_column']} OFFSET {i * factor} ROWS FETCH NEXT {factor} ROWS ONLY;"))
