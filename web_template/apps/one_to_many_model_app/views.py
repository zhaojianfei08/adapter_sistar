...
@author_book_app.route(f'/api/{ONE_SIDE_API_NAME}/', defaults={'id': None}, methods=['GET', 'POST'])
@author_book_app.route(f'/api/{ONE_SIDE_API_NAME}/<int:id>', methods=['GET', 'POST', 'PUT', 'DELETE'])
def get_one_side_items(id):
    if request.method == 'GET':
        many_info = MANY_SIDE_MODEL_NAME.query.filter_by(is_deleted=False).all()
        _id, _name = ONE_TO_MANY_DISPLAY
        many_info_display = [[info.to_dict().get(_id), info.to_dict().get(_name)] for info in many_info]
        if id:
            # 查询单条记录
...
@author_book_app.route(f'/api/{MANY_SIDE_API_NAME}/', defaults={'id': None}, methods=['GET', 'POST'])
@author_book_app.route(f'/api/{MANY_SIDE_API_NAME}/<int:id>', methods=['GET', 'POST', 'PUT', 'DELETE'])
def get_many_side_items(id):
    if request.method == 'GET':
        ...
        if id:
            ...
            query = MANY_SIDE_MODEL_NAME.query.filter_by(is_deleted=False)
            ...
            return render_template(f'{MANY_SIDE_TEMPLATE_NAME}.html', form_info=MANY_SIDE_FORM_INFO, total=items.total,
                                   ...
                                   per_page=items.per_page,
                                   items=result,
                                   page_obj=items, one_info_display=one_info_display)
    elif request.method == 'POST':
        data = request.json
