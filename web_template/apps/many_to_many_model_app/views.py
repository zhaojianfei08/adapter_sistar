...
@user_role_app.route(f'/api/{MANY_SIDE_A_API_NAME}/', defaults={'id': None}, methods=['GET', 'POST'])
@user_role_app.route(f'/api/{MANY_SIDE_A_API_NAME}/<int:id>', methods=['GET', 'POST', 'PUT', 'DELETE'])
def get_side_A_items(id):
    if request.method == 'GET':
        ...
        if id:
            ...
            total_page += 1
            return render_template(f'{MANY_SIDE_A_TEMPLATE_NAME}.html', form_info=MANY_SIDE_A_FORM_INFO, total=items.total,
                                   page=items.page,
...
@user_role_app.route(f'/api/{MANY_SIDE_B_API_NAME}/', defaults={'id': None}, methods=['GET', 'POST'])
@user_role_app.route(f'/api/{MANY_SIDE_B_API_NAME}/<int:id>', methods=['GET', 'POST', 'PUT', 'DELETE'])
def get_side_B_items(id):
    if request.method == 'GET':
        many_side_A_info = MANY_SIDE_A_MODEL_NAME.query.filter_by(is_deleted=False).all()
        _x, _y = B_TO_A_DISPLAY
        many_side_A_info_display = [[info.to_dict().get(_x), info.to_dict().get(_y)] for info in many_side_A_info]
        if id:
            # 查询单条记录
            item = MANY_SIDE_B_MODEL_NAME.query.filter_by(id=id, is_deleted=False).first()
            # 查询单条记录的所有的多对多字段
            many_side_program_id_list = [many_side_obj.to_dict()[A_LINK_B_PROGRAM_ID] for many_side_obj in
                                         getattr(item, B_LINK_A_COLUMN)]
            ...
            for item in items:
                _d = {}
                many_side_A_display_list = [many_side_obj.to_dict()[B_LINK_A_DISPLAY_NAME] for many_side_obj in
                                          getattr(item, B_LINK_A_COLUMN).all()]
                _d.update(item.to_dict())
                _d.update({B_LINK_A_COLUMN: many_side_A_display_list})
                result.append(_d)


            return render_template(f'{MANY_SIDE_B_TEMPLATE_NAME}.html', form_info=MANY_SIDE_B_FORM_INFO, total=items.total,
                                   page=items.page,
                                   total_page=total_page,
                                   per_page=items.per_page,
                                   items=result,
                                   page_obj=items, many_side_A_info_display=many_side_A_info_display)
    elif request.method == 'POST':
        data = request.json
        ...
        for many_side_id in new_data_copy[B_LINK_A_COLUMN]:
            # 获取多方对象

            many_side_obj = MANY_SIDE_A_MODEL_NAME.query.get(many_side_id)
            print(many_side_obj)
            if many_side_obj:
                # 将多方对象中append进来
                getattr(new_item, B_LINK_A_COLUMN).append(many_side_obj)
        new_item.save()
